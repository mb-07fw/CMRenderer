...
#define CM_KEYSTATE_WAS_PRESSED_BIT 0x40 // Custom flag to track if a key was pressed previously. My bit now...
...
namespace CMRenderer
{
		...
		: m_CMLoggerRef(cmLoggerRef), m_KeyStates() // Not sure if std::array initializes to defaults automatically...
	{
		...
		if (m_CurrentInputLocale == nullptr)
			m_CMLoggerRef.LogFatalNL(L"CMKeyboard [()] | Failed to retrieve the current input locale.");
