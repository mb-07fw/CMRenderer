...
#pragma region DXDevice
	DXDevice::DXDevice(CMCommon::CMLoggerWide& logger) noexcept
		: m_Logger(logger)
	{
		m_Logger.LogInfoNL(L"DXDevice [()] | Constructed.");
	}
	...
	DXDevice::~DXDevice() noexcept
	{
		...
		m_Logger.LogInfoNL(L"DXDevice [~()] | Destroyed.");
	}
	...
	void DXDevice::Create() noexcept
	{
		m_Logger.LogFatalNLIf(
			m_Created,
			L"DXDevice [Create] | Attempted to create after the device has already been created."
		);

		CreateDevice();

		m_Logger.LogInfoNL(L"DXDevice [Create] | Created.");
	...
	void DXDevice::Release() noexcept
	{
		m_Logger.LogFatalNLIf(
			!m_Created,
			L"DXDevice [Release] | Attempted to release before creation has occured."
		);

		m_Logger.LogFatalNLIf(
			m_Released,
			L"DXDevice [Release] | Attempted to release after DXDevice was released previously."
		);
		...
		m_Logger.LogInfoNL(L"DXDevice [Release] | Released.");
	...
	void DXDevice::CreateDevice() noexcept
	{
		...
		m_Logger.LogFatalNLIf(FAILED(hResult), L"DXDevice [CreateDevice] | Failed to upgrade device to ID3D11Device1.");

		// Upgrade device context to ID3D11DeviceContext1. (D3D11.1)
		hResult = pBaseContext.As(&mP_Context);

		m_Logger.LogFatalNLIf(FAILED(hResult), L"DXDevice [CreateDevice] | Failed to upgrade device context to ID3D11DeviceContext1.");
	}
#pragma endregion

#pragma region DXFactory
	DXFactory::DXFactory(CMCommon::CMLoggerWide& logger) noexcept
		: m_Logger(logger)
	{
		m_Logger.LogInfoNL(L"DXFactory [()] | Constructed.");
	}
	...
	DXFactory::~DXFactory() noexcept
	{
		...
		m_Logger.LogInfoNL(L"DXFactory [~()] | Destroyed.");
	}

	void DXFactory::Create(DXDevice& device) noexcept
	{
		m_Logger.LogFatalNLIf(
			m_Created,
			L"DXFactory [Create] | Attempted to create factory after DXFactory has already been created."
		);

		m_Logger.LogFatalNLIf(
			!device.IsCreated(),
			L"DXFactory [Create] | Attempted to create factory before the provided DXDevice was created."
		);

		HRESULT hResult = CreateDXGIFactory1(IID_PPV_ARGS(&mP_Factory));
		m_Logger.LogFatalNLAppendIf(
			FAILED(hResult),
			L"DXFactory [Create] | Failed to create factory : ",
			WindowsAPI::Utility::TranslateError(hResult)
		);

		m_Logger.LogInfoNL(L"DXFactory [Create] | Created.");
	...
	void DXFactory::Release() noexcept
	{
		m_Logger.LogFatalNLIf(
			!m_Created,
			L"DXFactory [Release] | Attempted to release factory before DXFactory has been created."
		);

		m_Logger.LogFatalNLIf(
			m_Released,
			L"DXFactory [Release] | Attempted to release factory after DXFactory has already been released."
		);

		mP_Factory.Reset();

		m_Logger.LogInfoNL(L"DXFactory [Release] | Released.");
...
#pragma region DXSwapChain
	DXSwapChain::DXSwapChain(CMCommon::CMLoggerWide& logger) noexcept
		: m_Logger(logger)
	{
		...
		m_Logger.LogInfoNL(L"DXSwapChain [()] | Constructed.");
	}
	...
	DXSwapChain::~DXSwapChain() noexcept
	{
		...
		m_Logger.LogInfoNL(L"DXSwapChain [~()] | Destroyed.");
	}

	void DXSwapChain::Create(const HWND hWnd, const RECT clientArea, DXFactory& factory, DXDevice& device) noexcept
	{
		m_Logger.LogFatalNLIf(
			m_Created,
			L"DXSwapChain [Create] | Attempted to create DXSwapChain after it has already been created."
		);

		m_Logger.LogFatalNLIf(
			!factory.IsCreated(),
			L"DXSwapChain [Create] | Attempted to create DXSwapChain before the provided DXFactory has been created."
		);

		m_Logger.LogFatalNLIf(
			!factory.IsCreated(),
			L"DXSwapChain [Create] | Attempted to create DXSwapChain before the provided DXDevice has been created."
		);

		m_Desc.OutputWindow = hWnd;

		HRESULT hResult = factory->CreateSwapChain(device.DeviceRaw(), &m_Desc, &mP_SwapChain);
		...
		m_Logger.LogInfoNL(L"DXSwapChain [Create] | Created.");
	}
	...
	void DXSwapChain::Release() noexcept
	{
		m_Logger.LogFatalNLIf(
			!m_Created,
			L"DXSwapChain [Release] | Attempted to release DXSwapChain before it has been created."
		);

		m_Logger.LogFatalNLIf(
			m_Released,
			L"DXSwapChain [Release] | Attempted to release DXSwapChain after it has already been released."
		);
		...
		m_Logger.LogFatalNLIf(
			FAILED(hResult),
			L"DXSwapChain [Release] | Failed to retrieve swap chain descriptor."
		);
		...
		m_Logger.LogInfoNL(L"DXSwapChain [Release] | Released.");
	}
...
#pragma region DXInfoQueue
	DXInfoQueue::DXInfoQueue(CMCommon::CMLoggerWide& logger) noexcept
		: m_Logger(logger)
	{
		m_Logger.LogInfoNL(L"DXSwapChain [()] | Constructed.");
	}
	...
	void DXInfoQueue::Create(DXDevice& device) noexcept
	{
		m_Logger.LogFatalNLIf(
			m_Created,
			L"DXInfoQueue [Create] | Attempted to create DXInfoQueue after creation has already occured."
		);

		m_Logger.LogFatalNLIf(
			!device.IsCreated(),
			L"DXInfoQueue [Create] | Attempted to create DXInfoQueue with a un-initialized device."
		);
		...
		m_Logger.LogInfoNL(L"DXInfoQueue [Create] | Created.");
	}
	...
	void DXInfoQueue::Release() noexcept
	{
		m_Logger.LogFatalNLIf(
			m_Released,
			L"DXInfoQueue [Release] | Attempted to release after DXInfoQueue has already been released."
		);
		
		m_Logger.LogFatalNLIf(
			!m_Created,
			L"DXInfoQueue [Release] | Attempted to release before DXInfoQueue has been created."
		);
		...
		m_Logger.LogInfoNL(L"DXInfoQueue [Release] | Released.");
	}
	...
	[[nodiscard]] bool DXInfoQueue::IsQueueEmpty() noexcept
	{
		bool notCreated = m_Logger.LogWarningNLIf(
			!m_Created,
			L"DXInfoQueue [IsQueueEmpty] | Attempted to check if info queue is empty "
			L"before DXInfoQueue is created."
		);
	...
	void DXInfoQueue::GetMessages(std::vector<std::wstring>& outMessages) noexcept
	{
		bool notCreated = m_Logger.LogWarningNLIf(
			!m_Created,
			L"DXInfoQueue [GetMessages] | Attempted to retrieve messages before DXInfoQueue "
			L"has been created."
		);
		...
		for (size_t i = 0; i < mP_InfoQueue->GetNumStoredMessages(); ++i)
		{
			...
			bool failed = m_Logger.LogWarningNLIf(
				FAILED(hResult),
				L"DXInfoQueue [GetMessages] | Failed to get a message in the info queue. "
				L"Skipping message..."
			);
