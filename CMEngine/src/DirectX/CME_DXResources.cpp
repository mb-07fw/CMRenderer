...
namespace CMEngine::DirectXAPI::DX11
{
	...
	[[nodiscard]] HRESULT DXBuffer::Create(DXDevice& device) noexcept
	{
		CM_ASSERT(!m_Created);
	...
	void DXVertexBuffer::Bind(DXDevice& device, UINT offset) noexcept
	{
		CM_ASSERT(Buffer() != nullptr);
		CM_ASSERT(IsCreated());

		device.ContextRaw()->IASetVertexBuffers(m_Register, 1u, BufferAddress(), &m_Stride, &offset);
	}
	...
	void DXIndexBuffer::Bind(DXDevice& device, UINT offset) noexcept
	{
		CM_ASSERT(Buffer() != nullptr);
		CM_ASSERT(IsCreated());
	...
	void DXConstantBuffer::BindVS(DXDevice& device, UINT offset) noexcept
	{
		CM_ASSERT(Buffer() != nullptr);
		CM_ASSERT(IsCreated());
	...
	void DXConstantBuffer::BindPS(DXDevice& device, UINT offset) noexcept
	{
		CM_ASSERT(Buffer() != nullptr);
		CM_ASSERT(IsCreated());
