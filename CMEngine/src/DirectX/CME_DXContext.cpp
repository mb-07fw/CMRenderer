...
#pragma region DXContextState
	...
	void DXContextState::SetCurrentShaderSet(DXShaderSetType shaderType) noexcept
	{
		m_Logger.LogFatalNLIf(
			!m_Device.IsCreated(),
			L"DXContextState [SetCurrentShaderSet] | Device is not created."
		);

		m_Logger.LogFatalNLIf(
			!m_ShaderLibrary.IsInitialized(),
			L"DXContextState [SetCurrentShaderSet] | Shader library is not initialized."
		);
		...
		m_Logger.LogFatalNLIf(
			...
			L"DXContextState [SetCurrentShaderSet] | Retrieved shader set pointer is expired."
		);
		...
		m_Logger.LogFatalNLIf(
			...
			L"DXContextState [SetCurrentShaderSet] | Retrieved shader set wasn't created previously."
		);

		m_Logger.LogFatalNLIf(
			pCurrentShaderSet->pVertexShader.Get() == nullptr,
			L"DXContextState [SetCurrentShaderSet] | Current shader set's vertex shader was nullptr."
		);

		m_Logger.LogFatalNLIf(
			pCurrentShaderSet->pPixelShader.Get() == nullptr,
			L"DXContextState [SetCurrentShaderSet] | Current shader set's pixel shader was nullptr."
		);

		m_Logger.LogFatalNLIf(
			pCurrentShaderSet->pInputLayout.Get() == nullptr,
			L"DXContextState [SetCurrentShaderSet] | Current shader set's input layout was nullptr."
		);
	...
	void DXContextState::SetCurrentModelMatrix(const DirectX::XMMATRIX& modelMatrix) noexcept
	{
		m_CurrentModelMatrix = modelMatrix;
		m_ModelMatrixSet = true;
	}

	void DXContextState::SetCurrentCameraData(const CMCameraData& cameraData) noexcept
	{
		m_CameraData = cameraData;
		m_CameraUpdated = true;
	}

	void DXContextState::SetCurrentCameraTransform(const CMCommon::CMRigidTransform& rigidTransform) noexcept
	{
	...
	void DXContextState::RebindCurrentShaderSet() noexcept
	{
		m_Logger.LogFatalNLIf(
			mP_CurrentShaderSet.expired(),
			L"DXContextState [RebindCurrentShaderSet] | No shader set is currently set."
		);
	...
	void DXContextState::UpdateCamera() noexcept
	{
		if (!m_CameraUpdated && !m_WindowResized)
		{
			m_Logger.LogWarningNL(
				L"DXContextState [UpdateCamera] | Attempted to update camera even "
...
#define COMMA ,
		  ...
		  CM_IF_NDEBUG_REPLACE(m_InfoQueue(logger))
	{
		CM_IF_DEBUG(
			...
			m_Logger.LogFatalNLIf(
				FAILED(hResult),
				L"DXContext [()] | Failed to retrieve a DXGI debug interface."
			);
		);
	...
	void DXContext::Init(const HWND hWnd) noexcept
	{
		...
		CM_IF_DEBUG(
			...
			m_Logger.LogFatalNLIf(
				...
				L"DXContext [Init] | Failed to initialize info queue."
			);
		);
	...
	void DXContext::SetShaderSet(DXShaderSetType setType) noexcept
	{
		m_Logger.LogFatalNLIf(
			...
			L"DXContext [SetShaderSet] | The provided DXShaderSetType should not be DXShaderSetType::INVALID."
		);
	...
	void DXContext::SetModelMatrix(const DirectX::XMMATRIX& modelMatrix) noexcept
	{
		m_State.SetCurrentModelMatrix(modelMatrix);
	}

	void DXContext::SetCamera(const CMCameraData& cameraData) noexcept
	{
		m_State.SetCurrentCameraData(cameraData);
	}

	void DXContext::SetCameraTransform(const CMCommon::CMRigidTransform& rigidTransform) noexcept
	{
...
#pragma endregion
	...
	void DXContext::ReportLiveObjects() noexcept
	{
		CM_IF_DEBUG(
			...
			m_Logger.LogFatalNLIf(
				FAILED(hResult),
				L"DXContext [DrawIndexed] | Failed to report live objects."
			)
		);
	}
	...
	[[nodiscard]] bool DXContext::IsFullscreen() noexcept
	{
		...
		m_Logger.LogFatalNLIf(
			FAILED(hResult),
			L"DXContext [IsFullscreen] | Failed to get fullscreen state."
		);
	...
	void DXContext::InitImGui(const HWND hWnd) noexcept
	{
		m_Logger.LogFatalNLIf(
			!m_Device.IsCreated(),
			L"DXContext [InitImGui] | Device was not created previously."
		);
	...
	void DXContext::ShutdownImGui() noexcept
	{
		m_Logger.LogFatalNLIf(
			!m_Initialized,
			L"DXContext [InitImGui] | Context was not initialized previously."
		);
	...
	void DXContext::CreateRTV() noexcept
	{
		m_Logger.LogFatalNLIf(
			mP_RTV.Get() != nullptr,
			L"DXContext [CreateRTV] | A RTV is still present."
		);

		m_Logger.LogFatalNLIf(
			mP_DSV.Get() != nullptr,
			L"DXContext [CreateRTV] | A DSV is still present."
		);
	...
	void DXContext::OnWindowResize() noexcept
	{
		...
		m_Logger.LogFatalNLIf(
			FAILED(hResult),
			L"DXContext [OnWindowResize] | Failed to retrieve swap chain description."
		);
		...
		m_Logger.LogFatalNLIf(
			FAILED(hResult),
			L"DXContext [OnWindowResize] | Failed to resize buffers."
		);
		...
		m_Logger.LogFatalNLIf(
			...
			L"DXContext [OnWindowResize] | CMFrameData constant buffer wasn't created previously."
		);
		...
		m_Logger.LogFatalNLIf(
			FAILED(hResult),
			L"DXContext [OnWindowResize] | Failed to update FrameData constant buffer."
		);
	...
	void DXContext::RebindCurrentShaderSet() noexcept
	{
		m_Logger.LogFatalNLIf(
			!m_Initialized,
			L"DXContext [RebindCurrentShaderSet] | Context isn't initialized."
		);
