...
namespace CMEngine
{
	...
	{
		spdlog::info("CMEngine Run!");

		constexpr float TargetFrameTime = 1000.0f / 60;

		float deltaTime = 0.0f;
		while (mP_Platform->Window()->IsRunning())
		{
			...
			std::this_thread::sleep_for(std::chrono::duration<float, std::milli>(TargetFrameTime - deltaTime));
		}
	}

	void CMEngine::Init() noexcept
	{
		spdlog::info("CMEngine Init!");

		Platform::LoadInterface(mP_Platform);
	...
	{
		Platform::Unload(mP_Platform);

		spdlog::info("CMEngine Shutdown!");
