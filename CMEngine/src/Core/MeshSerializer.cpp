...
{
	MeshSerializer::MeshSerializer(Common::LoggerWide& logger) noexcept
		: m_Logger(logger)
	{
	}

	[[nodiscard]] bool MeshSerializer::SaveMesh(const std::filesystem::path& filePath, const MeshData& meshData) noexcept
	{
		YAML::Node root = YAML::convert<MeshData>::encode(meshData);
	...
	void MeshSerializer::LoadMesh(const std::filesystem::path& filePath, MeshData& outData) noexcept
	{
	...
	void MeshSerializer::EnforceValidMeshDescriptor(const MeshDescriptor& descriptor, const std::filesystem::path& filePath) noexcept
	{
		constexpr std::wstring_view FuncTag = L"MeshSerializer [EnforceValidMeshDescriptor] | ";
		...
		m_Logger.LogFatalNLFormattedIf(
			descriptor.VertexByteStrideHint == TypeHint::INVALID,
			FuncTag,
		...
		m_Logger.LogFatalNLFormattedIf(
			descriptor.IndexByteStrideHint == TypeHint::INVALID,
			FuncTag,
		...
		for (const VertexAttribute& vertexAttribute : descriptor.Attributes)
		{
		...
		m_Logger.LogFatalNLFormattedIf(
			descriptor.BinaryBeginBytePos == Common::Constants::U32_MAX_VALUE,
			FuncTag,
	...
	void MeshSerializer::ParseNodes(
		MeshData& outData,
		const YAML::Node& root,
		const std::filesystem::path& filePath
	) noexcept
	{
		constexpr std::wstring_view FuncTag = L"MeshSerializer [ParseNodes] | ";
		...
		YAML::Node meshNode = root["SerializedMesh"];
	...
	void MeshSerializer::LoadMeshDescriptor(
	...
	{
		constexpr std::wstring_view FuncTag = L"MeshSerializer [LoadMeshDescriptor] | ";
	...
	void MeshSerializer::LoadMeshData(
	...
	{
		constexpr std::wstring_view FuncTag = L"MeshSerializer [LoadMeshData] | ";
	...
	void MeshSerializer::ParseVertexStrideNodes(
		MeshDescriptor& outDescriptor,
		const std::filesystem::path& filePath,
	...
	{
		constexpr std::wstring_view FuncTag = L"MeshSerializer [ParseVertexStrideNodes] | ";
		...
		if (vertexStrideHintNode.IsDefined())
		{
			...
			m_Logger.LogFatalNLFormattedIf(
				outDescriptor.VertexByteStrideHint == TypeHint::INVALID,
				FuncTag,
				G_ERMW_DESCRIPTOR_VERTEX_BYTE_STRIDE_HINT_PARSE_INVALID,
				filePathWStr
			);
		...
		outDescriptor.VertexByteStrideHint = TypeHint::NONE;
	...
	void MeshSerializer::ParseIndexStrideNodes(
		MeshDescriptor& outDescriptor,
		const std::filesystem::path& filePath,
	...
	{
		constexpr std::wstring_view FuncTag = L"MeshSerializer [ParseIndexStrideNodes] | ";
		...
		if (indexStrideHintNode.IsDefined())
		{
			...
			m_Logger.LogFatalNLFormattedIf(
				outDescriptor.IndexByteStrideHint == TypeHint::INVALID,
				FuncTag,
				G_ERMW_DESCRIPTOR_INDEX_BYTE_STRIDE_HINT_PARSE_INVALID,
				filePathWStr
			);
		...
		outDescriptor.IndexByteStrideHint = TypeHint::NONE;
	...
	void MeshSerializer::ParseAttributesNode(
		MeshDescriptor& outDescriptor,
		const std::filesystem::path& filePath,
		const YAML::Node& attributesNode
	) noexcept
	{
		constexpr std::wstring_view FuncTag = L"MeshSerializer [ParseAttributesNode] |";
		...
		for (const auto& node : attributesNode)
		{
			 ...
			 * and implies the data is not compatible with a preset TypeHint.
			 */
			if (!typeHintNode)
			{
				outDescriptor.Attributes.emplace_back(name, TypeHint::NONE, byteOffset);
				continue;
			}

			std::string typeHintStr = typeHintNode.as<std::string>();
			bool parsed = false;
			TypeHint typeHint = StringToTypeHint(typeHintStr, parsed);
