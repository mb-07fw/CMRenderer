...
#pragma region Shader Library
	...
	{
		if (!std::filesystem::exists(CM_PLATFORM_PATH_SHADER_DIRECTORY))
			spdlog::critical(
				...
				CM_PLATFORM_PATH_SHADER_DIRECTORY "`"
			);
		else if (!std::filesystem::exists(CM_PLATFORM_PATH_COMPILED_SHADER_DIRECTORY))
			spdlog::critical(
				...
				CM_PLATFORM_PATH_COMPILED_SHADER_DIRECTORY "`"
			);

		std::vector<ShaderData> shaderData;
		for (const auto& entry : std::filesystem::directory_iterator(CM_PLATFORM_PATH_COMPILED_SHADER_DIRECTORY))
		{
			if (!entry.exists())
			{
